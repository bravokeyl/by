<?php
/**
 *  sumerian Sanitize Functions.
 */

if ( ! function_exists( 'sumerian_sanitize_checkbox' ) ) {
    function sumerian_sanitize_checkbox( $input ) {
        if ( $input == 1 ) {
            return 1;
        } else {
            return 0;
 ...
 * @return string
 */
function sumerian_sanitize_css($string) {
...
function sumerian_sanitize_color_alpha( $color ){
 ...
 */
function sumerian_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'coloralpha':
                        $data[ $i ][ $id ] = sumerian_sanitize_color_alpha( $value );
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  sumerian_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
...
function sumerian_sanitize_file_url( $file_url ) {
    $output = '';
 ...
 * @return bool
 */
function sumerian_hero_fullscreen_callback ( $control ) {
    if ( $control->manager->get_setting('sumerian_hero_fullscreen')->value() == '' ) {
        return true;
    } else {
        return false;
    }
...
function sumerian_sanitize_select( $input, $setting ){
...
function sumerian_sanitize_number( $input ) {
    return balanceTags( $input );
}

function sumerian_sanitize_hex_color( $color ) {
    if ( $color === '' ) {
        return '';
    }
    if ( preg_match('|^#([A-Fa-f0-9]{3}){1,2}$|', $color ) ) {
...
function sumerian_sanitize_text( $string ) {
    return wp_kses_post( balanceTags( $string ) );
}

function sumerian_sanitize_html_input( $string ) {
    return wp_kses_allowed_html( $string );
}

function sumerian_showon_frontpage() {
