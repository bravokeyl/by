...
<?php if ( ! $disable ) { ?>
    ...
    <?php } ?>
    <?php do_action('sumerian_section_before_inner', 'gallery'); ?>

    <div class="g-layout-<?php echo esc_attr( $layout ); ?> <?php echo esc_attr( apply_filters( 'sumerian_section_container_class', 'container', 'gallery' ) ); ?>">
        ...
        <?php
        $readmore_link = get_theme_mod( 'sumerian_g_readmore_link' );
        $readmore_text = get_theme_mod( 'sumerian_g_readmore_text', esc_html__('View More', 'sumerian') );
        if ( $readmore_link ) {
        ?>
