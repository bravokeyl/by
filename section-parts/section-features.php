<?php
$id       = get_theme_mod( 'sumerian_features_id', esc_html__('features', 'sumerian') );
$disable  = get_theme_mod( 'sumerian_features_disable' ) == 1 ? true : false;
$title    = get_theme_mod( 'sumerian_features_title', esc_html__('Features', 'sumerian' ));
$subtitle = get_theme_mod( 'sumerian_features_subtitle', esc_html__('Why choose Us', 'sumerian' ));
if ( sumerian_is_selective_refresh() ) {
    $disable = false;
}
$data  = sumerian_get_features_data();
if ( !$disable && !empty( $data ) ) {
    $desc = get_theme_mod( 'sumerian_features_desc' );
?>
...
<?php } ?>
    ...
    <div class="<?php echo esc_attr( apply_filters( 'sumerian_section_container_class', 'container', 'features' ) ); ?>">
        ...
        <div class="section-content">
            <div class="row">
            <?php
            $layout = intval( get_theme_mod( 'sumerian_features_layout', 3 ) );
            foreach ( $data as $k => $f ) {
                $media = '';
                ...
                if ( $f['icon_type'] == 'image' && $f['image'] ){
                    $url = sumerian_get_media_url( $f['image'] );
                    if ( $url ) {
    ...
    <?php do_action('sumerian_section_after_inner', 'features'); ?>
